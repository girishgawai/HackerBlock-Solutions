...
public class Rat_Chases_Its_Cheese_OPTIMIZED {
	static boolean flag;
	...
	private static void ratWay(char[][] maze, int cr, int cc, int[][] ans) {
		...
		// agr diagonals calls v lgane ko bola toh 8 calls hoge time complexity bdhegi so array use krna 2 arrays 8 size ke R & C
		//		ratWay(maze, cr+1, cc, ans);
		//		ratWay(maze, cr-1, cc, ans);
		//		ratWay(maze, cr, cc+1, ans);
		//		ratWay(maze, cr, cc-1,  ans);
		int[] c = {0,0,1,-1};			// column ka array bnaya upr ke statements se agr val 1 or 0 rhi toh
		int[] r = {1,-1,0,0};			// row ka array bnaya upr ke statements se

		for (int i = 0; i < c.length; i++) {
			ratWay(maze, cr+r[i], cc+c[i], ans);
		}
